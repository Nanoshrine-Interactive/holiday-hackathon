...
export function LensAuth() {
  ...
  useEffect(() => {
    const checkAccounts = async () => {
      if (!isConnected || !address || !isSignedIn) return
      setStatus('checking')
          ...
          const accountPromises = accountsResult.value.items.map(async (item) => {
            ...
            if (!accountResult.isErr() && accountResult.value !== null) {
              ...
              return {
                ...
                id: metadata?.id || item.account.address, // Fallback to address if id is null
                name: metadata?.name || null,
                picture: metadata?.picture || null
              } as AccountMetadata // Explicitly cast to AccountMetadata
            }
            return null
          })

          const resolvedAccounts = (await Promise.all(accountPromises)).filter(
            (account): account is AccountMetadata => account !== null // Ensure the type is narrowed to AccountMetadata
          )
  ...
  const handleLogin = async () => {
    if (!isConnected || !address || !isSignedIn || !selectedAccount) return
    setStatus('signing')
    setError(null)
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {isConnected && !isSignedIn && <p className="text-blue-500 text-sm">Please sign-in with your wallet first</p>}

      {isConnected && isSignedIn && accounts.length > 0 && !selectedAccount && (
      ...
      {isConnected && isSignedIn && selectedAccount && !isAuthenticated && (
        <button
          onClick={handleLogin}
          disabled={isLoading}
